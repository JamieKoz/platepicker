...
export const useMeasurementStore = defineStore('measurement', () => {
  ...
  // Actions
  async function fetchMeasurement(page = 1) {
    try {
    ...
    const params: Record<string, string | number> = {
      ...
      if (Array.isArray(response.data)) {
        ...
        measurement.value = response.data.data || response.data;
  ...
  async function searchMeasurement(query: string) {
    currentSearch.value = query;
    return fetchMeasurement(1);
  }
  ...
  async function createMeasurement(name: string) {
    try {
  ...
  async function updateMeasurement(id: number, name: string) {
    try {
